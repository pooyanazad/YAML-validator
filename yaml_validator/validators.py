    ...
    try:
        ...
        return [
            ValidationIssue(
                tool="yaml",
                severity=Severity.CRITICAL,
                message=f"File reading error: {e!s}",
                file_path=file_path,
            )
        ]
    ...
    try:
        ...
        issues.append(
            ValidationIssue(
                tool="yaml",
                severity=Severity.CRITICAL,
                message=f"File encoding error (not valid UTF-8): {e!s}",
                file_path=file_path,
            )
        )
    except OSError as e:
    ...
    try:
        ...
        issues.append(
            ValidationIssue(
                tool="yamllint",
                severity=Severity.HIGH,
                message=f"yamllint execution failed: {e!s}",
                file_path=file_path,
            )
        )
    ...
    try:
            ...
            try:
                ...
                issues.append(
                    ValidationIssue(
                        tool="checkov",
                        severity=Severity.MEDIUM,
                        message=f"Failed to parse checkov output: {e!s}",
                        file_path=file_path,
                    )
                )
        ...
        issues.append(
            ValidationIssue(
                tool="checkov",
                severity=Severity.HIGH,
                message=f"checkov execution failed: {e!s}",
                file_path=file_path,
            )
        )
